...
from .forms import CommentSearch, ContactForm, RegisterForm
...
# (24) - Zadanie 9 - modyfikacja redirect z logowaniem po rejestracji!
def register(request):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            messages.success(request, f'Konto dla {user.username} zostało utworzone i jesteś zalogowany.')
            return redirect('profile') # Przekierowujemy na stronę profilu
    else:
...
@login_required
def profile(request):
    """Widok profilu użytkownika - dostępny tylko dla zalogowanych użytkowników.
...
# (24) Zadanie 10
@staff_member_required # dekorator już sam sprawdza is_staff=True
