...
# Handler do obsługi GET /products - zwraca listę wszystkich produktów z paginacją
async def get_products(request):
    # Odczytujemy parametry page i limit z query, domyślnie page=1, limit=10
    page = int(request.query.get("page", 1))
    limit = int(request.query.get("limit", 10))
    # Obliczamy offset na podstawie strony i limitu
    offset = (page - 1) * limit

    session_factory = request.app["db_session_factory"]
    async with session_factory() as session:
        async with session.begin():
            # Pobieramy produkty z bazy z uwzględnieniem offsetu i limitu (paginacja)
